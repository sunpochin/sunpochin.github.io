...
為避免將求職履歷/作品集直接混入一般終端使用者的首頁，且同時兼顧個人生活連結（社交舞、音樂、筆記翻譯）與核心服務（照護記）的清晰區隔，將網站劃分為：

```text
sunpochin.github.io/
├── index.html       # [一般使用者入口] 多區塊服務與生活門牌 (無履歷求職干擾，極速門牌)
│   ├── [Section 1] 🩺 核心照護與數位工具 (照護記 CareTrail)
...
* **一般使用者首頁 (`/`)**：
  - **核心照護與數位工具**：單獨顯眼放置正式產品（如照護記 `caretrail.vercel.app`），標註「正經事 Core」。
...
| :--- | :--- |
| **將社交舞與生活連結混入照護記同一個列表** | 會破壞核心照護工具（照護記）的嚴肅感與明確性，使用者無法快速區分專業工具與個人休閒筆記。 |
